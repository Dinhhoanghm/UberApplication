...
export class MapComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  private initMap(): void {
    ...
    this.map = L.map('map', {
      zoom: 14,
      center: latLng(21.0285, 105.8542), // Center the map on Hanoi, Vietnam
      zoomControl: false,
  ...
  private drawRoute(departure: Location, destination: Location, driver = this.isDriver): void {
    ...
    this.route = L.Routing.control({
      ...
      plan: L.Routing.plan([this.makeMarker(departure).getLatLng(), this.makeMarker(destination).getLatLng()], {
      ...
      waypoints: [this.makeMarker(departure).getLatLng(), this.makeMarker(destination).getLatLng()],
    }).addTo(this.map);
    ...
    if (this.isDriver) {
      this.toDepartureRoute = L.Routing.control({
        ...
        show: true,
        routeWhileDragging: true,
        plan: L.Routing.plan([this.carMarker.getLatLng(), this.makeMarker(departure).getLatLng()], {
          createMarker: function (i: number, waypoint: L.Routing.Waypoint) {
            let text;
            if (i == 0) text = departure.address;
            else text = destination.address;
        ...
        waypoints: [this.carMarker.getLatLng(), this.makeMarker(departure).getLatLng()],
      }).addTo(this.map);
    ...
    const trackDriverTimer = setInterval(() => {
      ...
      if (!this.mapService.passengerRideInProgress) {
        ...
        this.driverService.getVehicle().subscribe((vehicle: Vehicle) => {
          ...
          if (this.lastThreeLocations.length == 3) {
            if (
              this.lastThreeLocations[0].equals(this.lastThreeLocations[1]) &&
              this.lastThreeLocations[1].equals(this.lastThreeLocations[2])
            ) {
              this.passengerService.getActiveRide().subscribe(
                (ride: RideCreated) => {
                  const b = 0;
