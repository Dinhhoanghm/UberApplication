    ...
    if (this.denyRideForm.valid) {
      this.driverService.denyRide(this.denyRideForm.value.reason).subscribe({
        next: (value) => {
          ...
          this.notificationService.createNotification('You have successfully denied the ride.', 3000);
        },
        error: (error) => {
          if (error.error.message.includes('reason')) {
            this.notificationService.createNotification(
              'You must provide a reason for denying the ride with a maximum of 500 characters.',
              5000
            );
          } else {
