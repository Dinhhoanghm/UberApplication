...
export class RideEstimatesComponent implements OnInit {
  ...
  modeButtonText = 'Mark on the map';
  ...
  private mapGeoSearchObjectToLocation(geoSearchObj: any): Location {
    return new Location(geoSearchObj.label, geoSearchObj.x, geoSearchObj.y);
  }
  ...
  private async filterDepartureSuggestions() {
    this.departureSuggestions = await this.provider
      .search({ query: this.searchForm.value.departure })
      .then((results: object[]) => this.filterToTop5Suggestions(results));
  }

  private async filterDestinationSuggestions() {
    this.destinationSuggestions = await this.provider
      .search({ query: this.searchForm.value.destination })
      .then((results: object[]) => this.filterToTop5Suggestions(results));
  }
  ...
  enableSearch() {
    this.modeButtonText = 'Mark on the map';
  ...
  disableSearch() {
    this.modeButtonText = 'Enter in fields';
    this.clearSearchForm();
