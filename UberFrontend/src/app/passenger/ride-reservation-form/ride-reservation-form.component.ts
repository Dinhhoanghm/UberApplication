...
export class RideReservationFormComponent {
  ...
  emailFormatErrorText = 'Incorrect email format!';
  ...
  reserve() {
    ...
    if (this.isReservation) {
      ...
      if (moment.duration(date.diff(now)).asMinutes() > 5 * 60) {
        this.notificationService.createNotification('You can reserve the ride a maximum of 5 hours in advance!', 3000);
      } else {
    ...
    this.passengerService
      .reserveRide(reservation)
      .pipe(
        catchError((error) => {
          if (error.status == 0) {
            console.error('Server communication failed');
          } else if (error.status == 400) {
            this.rideReservationFailureMessage = 'You cannot order a new ride if you have a previous one!';
          } else {
            console.log(error);
            this.rideReservationFailureMessage = 'The system cannot find a driver. The order is declined.';
          }
