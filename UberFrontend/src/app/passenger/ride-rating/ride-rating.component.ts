...
export class RideRatingComponent implements OnInit {
  ...
  review() {
    ...
    const reviewer = new UserIdEmail(
      parseInt(<string>sessionStorage.getItem('user_id')),
      <string>sessionStorage.getItem('user_email')
    );
    ...
    if (this.isDriver) {
      this.passengerService.reviewDriver(review, this.rideId).subscribe();
    } else {
      this.passengerService.reviewVehicle(review, this.rideId).subscribe();
    }

    this.notificationService.createNotification('Rating has been successfully submitted!', 3000);
    this.close();
  }
}
