...
@Component({
  ...
  styleUrls: [
    '../../passenger/passenger-account/passenger-account.component.css',
    './admin-create-driver.component.css',
    '../../app.component.css',
  ],
})
export class AdminCreateDriverComponent implements OnInit {
  ...
  toggleUpdateMode() {
    if (this.createDriverForm.valid) {
      ...
      this.notificationService.createNotification('You must fill out all fields!', 5000);
    }
  }
  ...
  createDriverAndVehicle() {
    ...
    this.adminService.sendCreateDriverRequest(request).subscribe({
      next: () => {
        this.notificationService.createNotification('Driver successfully created.', 5000);
      },
      error: (error) => {
        if (error.error.message.includes('email')) {
          this.notificationService.createNotification('Email address is taken or invalid.', 5000);
        } else if (error.error.message.includes('changes')) {
          this.notificationService.createNotification('No data was modified.', 5000);
        } else if (error.error.message.toLowerCase().includes('file')) {
          this.notificationService.createNotification('You must select a valid image smaller than 5MB.', 5000);
        } else if (error.error.message.includes('surname')) {
          this.notificationService.createNotification('Surname cannot exceed 100 characters.', 5000);
        } else if (error.error.message.includes('name')) {
          this.notificationService.createNotification('Name cannot exceed 100 characters.', 5000);
        } else if (error.error.message.includes('phone')) {
          this.notificationService.createNotification('Phone number must be valid.', 5000);
        } else if (error.error.message.includes('address')) {
          this.notificationService.createNotification('Address cannot exceed 100 characters.', 5000);
        } else {
          this.notificationService.createNotification('An error occurred: ' + error.error.message, 5000);
        }
      },
    });
    ...
    this.adminService.sendCreateVehicleRequest(request2).subscribe({
      next: () => {
